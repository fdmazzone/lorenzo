...
\end{equation*}
Si  realizamos la sustitución $v=x'$ tendremos el sistema\comment{No es una forma muy habitual de denotar un sistema, tengo miedo que no se entienda, te sugiero lo verde (use el paquete \texttt{ empheq} que es bastante comodo para escribir ecuaciones}
...
\end{equation*}
donde el término de la derecha del producto escalar es una medida vectorial.  En \cite{distel} se define medida vectorial para espacios de Banach $X$  \textcolor{red}{y?...no sé si este comentario es conveniente aquí...}. \comment{Para mi tenes que poner todo lo que tenes sobre medidas vectoriales desde aca y lo de la sección siguiente  en el capítuo anterior de todos los prerequisitos.} \normalmarginpar\comment{ Empezaría recordando la definición del concepto de meddia vectorial. Formulala en un espacio de Banach y después particularizala a $\rr^m$ y la norma del caminante} Cuando $X$ es un espacio euclídeo  $m$-dimensional, una medida vectorial se puede pensar como un vector de medidas, es decir $\nu=(\nu_1,\nu_2,...,\nu_m)$ donde cada $\nu_i$ es una medida de Borel con signo.  La variación total de una medida vectorial $\nu$ se define para un conjunto de Borel $E$, (ver definición 4 de \cite{distel}, tomando $\rr^m$ con la norma de $l^1$) como 
...
El siguiente resultado nos permite expresar la variación total de una medida vectorial de una manera simple. \comment{Acota que si bien el resultado es bastante esperable no se encontró su enunciado y demostración en la literatura y por ese motivo  incluis la demostración (de paso así decís que la demostración es de tu autoría  
 )} 
...
\begin{proof}[Dem.]
    ...
    \end{split}
    \end{equation*}
    \textcolor{red}{En la frase que sigue se mezcla la descomposición con las componentes (o como se llamen...) de la descomposición.\comment{No entiendo que queres decir}}
    
\replaced{Para demostrar la desigualdad que resta, sea}{Sea} $E_i^{+}$ y $E_i^-$  la descomposición de Hahn del conjunto $E$ respecto de la medida $\nu_i$ (ver \cite{folland}), entonces \begin{equation}\label{eq:med_vec 1}
    |\nu_i|(E)=\nu_i(E_i^+)-\nu_i(E_i^-).
\end{equation}    
...
\begin{proof}[Dem.]
    El resultado se obtiene por aplicación de la Observación \eqref{obs:R-D}. \comment{Al menos diría que $H=(h_1,\ldots,h_m)$ y que,  como en el caso de medidas escalares, la función $H$ es la Derivada de Radon-Nikodym de $\nu$ respecto a $\|\nu \|$ y que se denotará $\frac{d\nu}{d\|\nu\|}$}
\end{proof}     

	
\subsection{Soluciones a ecuaciones con medidas vectoriales}\reversemarginpar \comment{Te sugiero elevar el rango de esto a sección, además te pedi llevar la sección que lo contiene al capítulo anterior} 
...
\begin{defi}\label{def-sol}
	Una solución de \eqref{eq:8} es una función $\varphi\in BV([0,T],\rr^n)$\reversemarginpar\comment{Me parece que funciones de variación acotada a valores vectoriales nunca fue definido} y continua a izquierda, tal que  
...
\begin{thm}\label{thm:m_equivalente} \normalmarginpar\comment{Me parece conveniente que recuerdes <<Sea $H$ como en ..... y $\mu$ bla bla>>}
Una solución de \eqref{eq:8} es también solución de    
\begin{equation}\label{eq:10}
	d\varphi=f(t,\varphi(t))d\mu,
\end{equation} 
donde $f=F[H]^*:[0,T]\times\rr^n\to \rr^n$ \reversemarginpar \comment{¿Qué es $H^*$?¿La transpuesta? ¿Es lo mismo que el $H^\tau$ de más abajo? no es usual el símbolo $\tau$ para la transpuesta.  No se por qué hay necesidad de transponer} y  $\mu$ es una medida de Borel positiva. 
\end{thm}
\begin{proof}[Dem.]
    De la \replaced{Proposición}{proposición} \eqref{prop:medida vectorial} \normalmarginpar\comment{Aca me parece que también necesitas la  Proposición \ref{ob1}} se tiene que existe $H:\rr^m\to\rr^n$ en  $L^1(||\nu||)$ tal que  podemos escribir la definición \eqref{eq:9} como 
...
De esta manera, podemos decir que \replaced{una}{la} \comment{\eqref{eq:8} no te determina una sola solución como para decir <<la solución>>} solución de \eqref{eq:8}, es también solución de un problema donde la medida que interviene no es vectorial. En efecto, si llamamos $f(t,x)=F(t,x)[H(t)]^\tau$ y $\mu=||\nu||$ tenemos 
\begin{equation*}
...
\newpage
 Si llamamos $\nu=(\lambda,\delta_{t_1},...,\delta_{t_r})$ y aplicamos el \replaced{Teorema}{teorema} \eqref{thm:m_equivalente} \reversemarginpar \comment{Para citar teoremas se usa el comando \texttt{\textbackslash ref} en lugar del \texttt{\textbackslash eqref} que es específico para ecuaciones. Ahora bien, para mi la conclusión que sigue es más consecuencia del Lema \ref{lem:medida vectorial} que del Teorema  \eqref{thm:m_equivalente} }  que \normalmarginpar  $||\nu||=\displaystyle\lambda+\sum_{k=1}^r\delta_{t_k}$. 
 
 \normalmarginpar \highlight[comment={para que se entienda más lo que vas a hacer}]{Vamos a calcular la derivada de Radon-Nikodym $d\nu/d\|\nu\|$.} Como
...
\replaced{Consecuentemente, una} {notando $H(t)=(1,\chi_{\{t_1\}},..,\chi_{\{t_r\}})$  la} solución de \eqref{eq:ejemplo 1} es también solución de la ecuación 

 	\begin{equation*}
	d\varphi=\left(F(t,x),g_1(t),...,g_r(t)\right)\added{\left[\frac{d\nu}{d\|\nu\|}\right]^\tau}\textcolor{red}{\cancel{H(t)^\tau}} \; d||\nu||.
\end{equation*} 
\replaced{que es una ecuación de la forma de la ecuación}{Si escribimos $f(t,x)=(F(t,x),g_1(t),...,g_r(t))H(t)^\tau$ y $||\nu||=\mu$ entonces  $\varphi$ es solución de} \eqref{eq:10}.

 \end{example}
%%%%%%%%%%%%%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%% otras soluciones %%%%%%%%%%%%%%%%%

En la definición  \ref{def-sol}, dijimos que una solución de la MDE \added{\eqref{eq:10}}
{\color{red}
\begin{equation*}
	\cancel{d\varphi=f(t,\varphi(t))d\mu,}
\end{equation*}
}
es una función $\varphi\in BV([0,T],\rr^n)$ \deleted{y} continua a izquierda, que cumple con la ecuación integral \eqref{eq:9}. Sin embargo, no es la única \textcolor{blue}{forma de definir  solución} \deleted{solución} que podemos \replaced{encontrar en la literatura de la materia }{ obtener} para esta ecuación. 

\highlight[comment={No entiendo. ¿Estas queriendo decir que las distintas definiciones no son equivalentes?}]{Dependiendo de  a qué   consideramos solución, ésta puede variar.} Si en la construcción de la medida de Lebesgue-Stieltjes (hecha en el capítulo $2$) tomamos funciones continuas a derecha, entonces la solución a \eqref{eq:10} será distinta\reversemarginpar\comment{Me parece que la medida de Lebesgue-Stieljes no cambia. La medida asociada es la misma sea continua para una lado o para otro. Lo que cambia es la integral $\int f(t,\varphi)d\nu$ }. 


\vspace{1.5cm}
ejemplo,\normalmarginpar\comment{Por qué no un \texttt{\textbackslash begin\{ejemplo\}}?}  consideremos el siguiente problema de valores iniciales
...
x(t)=\left\{\begin{array}{ccc}
    ...
    \end{array}\right.
\end{equation*}
\replaced{Si modificasemos nuestra Definición \ref{def-sol} exigiendo que una solución sea continua a derecha,  la solución sería}{En cambio,  la solución continua a derecha es}\comment{Nunca hablamos de soluciones continuas a derecha, porque en la Definición \ref{def-sol} dice expresamente que son continuas a izquierda. Por eso te propongo lo que escribí en verde}
\begin{equation*}
x(t)=\left\{\begin{array}{ccc}
        x_0 & si & t\in[0,1)  \\
...
\end{equation}
donde $v_k$ son continuas y  $v'_k\to \dfrac{d\mu}{d\lambda}$ \added{en la topología debil-${}^\ast$ de $C([0,T])^\ast$}

\newpage

Veamos este \comment{le daría más relevancia a este ejemplo con un \texttt{\textbackslash begin\{ejemplo\}}. Explicaría más que queres decir con <<este tipo de solución>>} tipo de solución para  el ejemplo \eqref{ej:soluciones}. La sucesión de funciones
\begin{equation*}
h_k(t)=\left\{\begin{array}{ccc}
    ...
    \end{array}\right.
\end{equation*}
son continuas y convergen \reversemarginpar\comment{Siempre que se dice <<converge>> hay que decir como} a la función de Heaviside $H_1$, cuya derivada es $\delta_1$. \normalmarginpar\highlight[comment={No es un luego, por que lo que sigue no se infiere de lo anterior}]{Luego} la solución a
\begin{equation}
\left\{\begin{array}{lc}
...
Como vemos podemos tener distintas soluciones para la misma ecuación, dependiendo de qué entendamos como solución. \comment{Poner cita, creo que el primero que se refirió a esto fue un tal Persson. Sugiero lo verde} Este fenómeno  {\color{green} fue descripto por J. Persson (ver\cite{persson1989regularization,S.T.Zavalishchin})}\deleted{se conoce} como la paradoja de \added{las } MDE.
 ...
 \todo[inline]{Para ser más precisos llamaría a esta sección ``Existencia de soluciones a problemas de valores iniciales para MDE''.  No pondría el teorema de cambio de variables en esta sección, le haría una sección propia a ese teorema.}
 
 
 
 
  En esta sección vamos a presentar algunos resultados, probados en \cite{P.Mazzone}, sobre existencia de soluciones locales \added{a problemas de valores iniciales}. 


  
  Al final, demostraremos un teorema de cambio de variables para integrales de Lebesgue-Stieltjes (L-S)\index[Simbolo]{(L-S)}. \comment{Esto,para mi,  debería ir en otra sección, en una propia para ese resultado}
...
\begin{defi}
    Sea $t_0\in [0,T)$, $h>0$,  $x_0\in \rr^n$ y $r>0$, diremos que $f:\Omega=[t_0,t_0+h]\times\rr^n\to\rr^{n}$ es localmente Lipschitz \index{localmente lipschits} \added{respecto a la segunda variables, }si  existe $ L>0$  tal que
...
\end{thm}
 ...
 \begin{defi}\comment{Esta definición no la usas en esta sección, recién la usas por primera vez, creo, en el Teorema \ref{th: P}. Mejor introducila en ese lugar.}  
...
\begin{thm}	\label{th:extensión}
	Sean $\mu$ una medida de Borel finita, $f$ \comment{Sería bueno especificar dominio y codominio de $f$, o decir que es igual que en otro enunciado} localmente Lipschitz respecto a la \added{segunda} variable \deleted{ vectorial} y $\varphi$  solución máxima de \eqref{Problema P} en $I=[t_0,t_1)$. Entonces una y sólo una de las siguientes afirmaciones es verdadera: 
	\begin{enumerate}[label=\upshape(\Roman*)]
		\item  Para todo $K\Subset\Omega$ existe $t_2\in I$ tal que $(t,\varphi(t))\notin K$ para todo $t\in(t_2,t_1)$.
		\item Existe el límite $x_1=\lim\limits_{t\to t_1^-}\varphi(t)$ \replaced{y este límite satisface que}{, tal que}  $(t_1,x_1)\in\Omega$ y 
...
\subsection{Teorema de Cambio de Variables}\index{Teorema de Cambio de Variables} \comment{Esta es una  subsección   de la sección  ``Existencia de Soluciones de MDE''. Me parece que no tiene tanto que ver con eso. La recategorizaría como sección}\reversemarginpar \comment{Te sugiero ingresar de siguiente modo en el glosario: \texttt{\textbackslash index\{Teorema!Cambio Variables\}}. Si vos ahora pones (por ejemplo) \texttt{\textbackslash index\{Teorema!Radon-Nikodym\}}  va a escribir una vez la palabra Teorema  y cada Teorema va a ser una subentrada de la palabra Teorema} 


\newpage
Vamos a demostrar una generalización \deleted{del Teorema de Cambio de Variables} \cite[Teorema 6.1]{P.Mazzone} \added{que es un resultado sobre cambios de variables en integrales respecto a medidas de Lebesgue-Stieltjes}. \highlight[comment={Todavía no apareció $F$, el que lee no le va a decir mucho}]{ , para el caso donde $|F''|<M$,}.
\added{En \cite[Teorema 6.1]{P.Mazzone} el resultado fue probado cuando cierta función $F$ es convexa, aquí lo extenderemos para funciones con derivada segunda acotada.} \replaced{Posteriormente}{ el cual} usaremos \added{nuestro resultado} para demostrar una desigualdad del tipo \added{de la desigualdad de } Gronwall.  


Empecemos enunciando el siguiente lema de cubrimiento demostrado en \cite[Corolario I, p 35]{Evans}.

\textcolor{red}{En la sección Bibliografía, habría que uniformizar la presentación de los autores (en algunos casos aparecen los nombres completos, en otros sólo la inicial).}

\begin{lem}[Lema de cubrimiento]
	\label{Lema de  cubrimiento}
	Sea $\mu$ una medida de Borel en $\rr^n$, y $\mathcal{F} $ cualquier colección de bolas cerradas. Sea $A$ el conjunto de los centros de las bolas en $\mathcal{F} $. Supongamos que $\mu(A)<\infty$ y \added{que}  $\inf\{r\mid B(a,r)\in \mathcal{F} \}=0$ para cada $a\in A$.  Entonces, para todo conjunto abierto $U\subset\rr^n$ existe una sucesión numerable $\mathcal{G} $ de bolas de $\mathcal{F} $ tal que 
	\begin{equation*}
...
Asumamos que $J\subset\rr$ es un intervalo abierto. Sea $F:J\to \rr$ una función diferenciable\comment{La derivamos dos veces, capaz que tengas que decir que es dos veces diferenciable} tal que $F'$ es acotada y absolutamente continua. Además,  supongamos que existe $M\geq 0$ tal que $F''\replaced{\geq}{\cancel{>}}-M$ \added{$\lambda$-en casi todo punto}. Entonces, para toda función $g:I\to J$ \added{no de}creciente y  continua a izquierda,  donde $I$ es un intervalo abierto, y para todo $A\in \mathcal{B}(I)$ vale que
...
\textcolor{blue}{El Teorema 6.1  de \cite{P.Mazzone} se deduce del Teorema  \ref{TCV} tomando $M=0$, pues $F$ sería convexa \comment{Por este comentario te pido que cambies la hipótesis a $F''\geq -M$, por que las funciones convexas satisface $F''\geq 0$ } y 
...
\begin{lem}\label{lem:mu_fog<<mu_g}
...
Para $[a,b)\subset I$, llamemos $x=g(a)$ e $y=g(b)$\comment{hay un pequeño detalle, como $I$ es abierto podría ocurrir que $b\notin I$}. Como $g$ es creciente entonces \replaced{$x\leq y$}{$x<y$}, y dado que $F'$ es continua, podemos aplicar el  teorema del valor medio del cálculo diferencial. Luego, existe en $c\in (x,y)$ tal que 

	$$ \dfrac{|F(y)-F(x)|}{y-x} =  |F'(c)| \leq \sup_{t\in J}\left|F'(t) \right|, $$
	y como \replaced{$|F'|\leq R$}{ $|F'|<R$ } para $R>0$, entonces
...
\end{obs}
\begin{proof}[Dem.]
    Para todo intervalo $(t_1,t_2)\subset I$, como $g$ es continua y creciente resulta que $(g(t_1),g(t_2))$ es un intervalo de $J$\comment{No se que queres decir, simpre $(g(t_1),g(t_2))$ es un intervalo, para cualquier función. Tampoco entiendo cuando decis ``de $J$''}, luego por \cite[Teorema 6.2.1]{Carter}
    \begin{multline}
...
\comment{Faltaría el caso general, $A\in \mathcal{B}(I)$}
...
Sea $A\in \mathcal{B}(I)$, vamos a ver primero el caso particular \normalmarginpar\comment{Me parece que después no usas que demostraste el caso particular, no se podría sacar el caso particular?} cuando
$A\cap D$ se reduce a un punto.
 Para cualesquiera $x$ e $y$ en $J$, por el teorema de Taylor (ver \cite[pg 13]{Evans}), existe $c$ entre $x$ e $y$ tal que
		...
		Como \replaced{$F''\geq -M$}{$F''>-M $} , \reversemarginpar\comment{hay que cambiar el $>$ por $\geq$ en todos lados.} entonces
...
\comment{Me parece que aca termino el caso particular y empezas con el general, deberías anunciarlo en el texto. Aunque insisto que me parece que tratar el caso particular no lo veo necesario}
Supongamos ahora que  $A\in \mathcal{B}(I)$ es un conjunto abierto cualquiera. Luego por el \replaced{Lema}{lema} \ref{lem: abs cont} y   \cite[Teorema 3.5]{folland}, para cualquier $\epsilon>0$ va a existir $\delta_1=\epsilon/R$ tal que si $\mu_g(A)< \delta_1$, entonces
...
Por otro lado, como $F'$ es uniformemente continua entonces existe $\delta_2<\epsilon$ tal que 
	...
	\begin{itemize}
 		\item $C$ es un conjunto abierto. \comment{me parece que no es cierto. No habrá sido $C=A\setminus{\overline{B_{j_0}}}$?,}
...
+\sum_{n=1}^{\infty}\int_{[t_n-\delta_n, t_n+\delta_n]}F'(g(t_n-\delta_n))dg(s).
	\end{multline*}
 Integrando en la primera sumatoria\comment{me parece que la primera sumatoria no es a la que te queres referir. No diría ``integrando'' porque la expresión ya aparece integrada}, y usando \eqref{eq:1} en la segunda,  tenemos que
	...
	\end{split}
\end{equation}
la ultima sumatoria la podemos acotar de la siguiente manera, \normalmarginpar\comment{Para mi las siguientes tres desigualdades las podes unir en una cadena, antes deberías decir que usas en la deducción. Así evitas escribis el primer miembro repetidamente}\reversemarginpar
\begin{multline*}
...
\end{multline*} 
Dado que    $\bigcup_{n=1}^{\infty} [t_n+\delta_n,t_n-\delta_n]$ cubre todo el conjunto $C$ salvo un conjunto $\mu_{F\circ g}$-nulo\normalmarginpar\comment{Para justificar que es $\mu_{F\circ g}$-nulo deberías citar el  Lema \ref{lem:mu_fog<<mu_g} } y \eqref{eq:cota de g(C)},  tenemos que 
\begin{multline*}
...
$$\mu_{F\circ g}(A)=\mu_{F\circ g}(B)+\mu_{F\circ g}(B_{j_0})+\mu_{F\circ g}(C).$$
 Además por \eqref{eq:medidas}\comment{Me parece más apropiado citar el Lema \ref{lem:mu_fog<<mu_g}} tenemos que
...
Reemplazando en \eqref{eq:3.3}\comment{Hay un problema con esta cita, fijate lo que escribiste en la ecuación que citas} tenemos que 
\begin{multline*}
...
\end{multline*}
cuando $\epsilon \to 0$, entonces $\delta_2\to 0$ y por lo tanto el conjunto $B=\left\lbrace s\in A\mid \mu_{g}(\{s\})\geq \delta_2\right\rbrace $ es igual\comment{No es igual, tiende en todo caso. Creo que habría que desarrollar un poquito este punto. Lo vemos en un encuentro} a $D\cap A=\left\lbrace s\in A\mid \mu_{g}(\{s\})> 0\right\rbrace$ y tenemos que
...
\begin{proof}[Dem.]
    ...
    \end{multline*}
    por el  \cite[Lema 1.17]{folland}\comment{No parece una referencia adecuada. Habría que ver como queda el capítulo de preliminares, quizás podes referenciar un resultado allí} podemos extender la igualdad a todo conjunto $A\in\mathcal{B}(I)$. Luego, de \eqref{eq:h} tenemos que
    \begin{equation*}
...
Como vimos en la sección anterior\textcolor{red}{,} bajo las hipótesis del \replaced{Teorema}{teorema} \ref{P-L}  existe  $\varphi_\alpha:[0,T]\to \rr^n$,  solución al problema de valores iniciales 
 \begin{equation}
	\left\lbrace \begin{array}{l}
...
\begin{lem}\label{Lema gronwall}
	Sea $\mu$ una medida finita, positiva y continua. Si $u\in L^1(\mu) $ \replaced{satisface}{tal} que 
	\begin{equation*}
...
\begin{proof}[Dem.]
 ...
 \added{Observando que la clase de los conjuntos $A$ que,  en lugar de $[t_1,t_2)$, satisfacen la identidad anterior es una $\sigma$-álgebra, intefrimos que la identidad vale para $A$ boreliano.}  Luego, por \st{el}  \cite[Teorema 3.5]{folland}\comment{Por qué no referis a tu Proposición \ref{prop:medida_int} que, además de estar en tu tesis, afirma de manera más precisa lo que queres decir},  $\mu_w\ll \mu$ y para toda función $g\in L^1(\mu)$ y cualquier conjunto $A$ de Borel, vale que 
...
\label{eq:lema 1}
 ...
 \begin{equation}
     ...
     \end{split}
 \end{equation}
 como $F$ es una función diferenciable, su derivada está acotada y es absolutamente continua y $F''>-2$ en $[0,T]$; y $h$ es una función continua pues la medida es continua \comment{Le agregaría una breve justificación de esta afirmación}. Entonces podemos aplicar el lema \ref{lem: g cont}, por lo cual tenemos 
 \begin{equation*}
 	\int_{[0,t)} F'(h(r)) \; d\mu= \mu_{F\circ h}([0,t))
 \end{equation*} 
%luego por \ref{obs3} 
y como $w\in L^1(\mu)$ entonces \comment{Nuevamente tu Proposición \ref{prop:medida_int}?}
\begin{equation*}
...
\end{equation*}
 Luego usando el Teorema \ref{T Partes} y que la medida $\mu$ es continua\comment{Habría que justificar que ${F\circ h}$ y $w$ son continuas} , tenemos que 
\begin{equation*}
...
\begin{enumerate}
\item \replaced{La función  $\mu_a$ es una medida pues si}{Sea  $\mu_a$ una medida y sea} $\{E_j\}_{j=1}^\infty\subset \mathscr{B}$ \added{es} una sucesión de conjuntos \added{mutuamente} disjuntos, entonces
\begin{equation*}
\begin{split}
...
\item La medida $\mu_a\leq\mu$.\comment{Agregaría una breve justificación }
...
\begin{thm}[\textbf{Desigualdad Mejorada de Gronwall}] \index{desigualdad de Gronwall}\label{TG} 	Sea $\mu$ una medida finita, positiva. Si $u\in L^1(\mu) $ \comment{No se si la notación lo dice, pero yo enfatizaría que $u$ es una función con valores en $\mathbb{R}$} tal que 
	\begin{equation*}
		u(t)\leq c+\int_{[0,t)}u(r) \; d\mu.
...
definida en \ref{def:sol}. De aquí  en adelante vamos a suponer que  $f:[0,T]\times\rr^n\to\rr^n$ está acotada, es decir $||f||_\infty<M$ y cumple con:
 \begin{enumerate}[label=\upshape(P-\arabic*),ref= (P-\arabic*)]
 	\item \label{pm1} Para cada $x\in\rr^n$, $f(t,x)$  $\mu$-medible en la variable $t$ y \highlight[comment={``continua'' respecto a qué?. No es lo mismo que decis abajo?}]{continua}.
 ...
 \end{enumerate} 
 Por el Teorema \ref{P-L}\comment{La idea sería citar el Teorema  \ref{th:picar}. Hay que chequear las hipótesis}, para cada $\alpha$ existe una solución $\varphi_\alpha$ del problema de valores iniciales \eqref{eq:Problema 0}. Veamos qué propiedades tiene esta solución.
\begin{prop}
...
Por otro lado, como \highlight[comment={Aca usas la acotación, pero me parece que con (P-3) y con el hecho que $\varphi_\alpha$ es acotada también sale}]{$||f||_\infty \leq M$}, para $s\leq t$ vale que
  	\begin{equation*}
...
Sea $\delta>0$ tal que  $\left[t_0-\delta,t_0+\delta\right]\subset\left[t_0-\frac{1}{n},t_0+\frac{1}{n}\right]$, entonces por la \replaced{Proposición}{proposición} \ref{acotación}, para $t\in \left[t_0-\delta,t_0+\delta\right]$ vale que
\begin{equation*}
...
El siguiente teorema muestra que las soluciones de \eqref{eq:Problema 0}  están definidas en todo $[0,T]$.
 ...
 \begin{thm}[\textbf{Dominio de la solución}]\label{th:P(T)}\comment{Este resultado se obtiene del Teorema  \ref{th:picar}}
...
%\begin{obs} \label{ob: cont P}
 ...
 \begin{thm} \label{th: P}
 	Sea $R>0$ y $f$ satisface las condiciones \ref{pm1} a \ref{pm4}, \added{la definición } \ref{fun-tele} para $B(0,R)$  y  además que
 	\begin{equation}
 ...
 \comment{Fijate de modificar algunos ``veamos'' lo escribiste varias veces en la página}Veamos primero que  $A$  es un conjunto cerrado respecto al $[0,T]$. Para ello, sea   $t_n$  una sucesión de $A$ que converge a $t$, veamos que $t\in A$.  Si para algún $n$, $t_n\geq t$ entonces $\forall s \in [0,t]\subset [0,t_n] $ vale que $\varphi_\alpha(s)\in\overline{B(0,R)}$,   por lo tanto \deleted{en} $t\in A$.  Si por el contrario $\forall n$ tenemos que $t_n < t$,  como $\varphi_\alpha$ es continua a izquierda, entonces $\lim\limits_{n\to \infty}\varphi_\alpha(t_n)=\varphi_\alpha(t)$ y como cada $\varphi_\alpha(t_n)\in \overline{B(0,R)}$ es inmediato que $\varphi_\alpha(t)\in \overline{B(0,R)}$, {\color{green} es decir $t\in A$}.  Por lo tanto, $A$ es cerrado.
  	...
  	\comment{Por definición si $t_{n_0}\notin A$ entonces existe $s\in [0,t_0]$ tal que $|\varphi_\alpha(s)|>R$. Lo que te señalo en verde arriba me parece que tiene la misma inexactitud }
  	
   \added{Como  $t_0\in A$ se tiene que $\varphi_\alpha(t_0)\leq R$.  Vamos a considerar tres casos:}{\color{cyan} Te sugiero usar números en lugar de viñetas para escribir cada caso, asi queda más claro que esos son los tres casos de los cuales hablamos aca}
   \deleted{Veamos que como $t_0\in A$, entonces $\varphi_\alpha(t_0)\in B(0,R)$ o  $\varphi_\alpha(t_0)\in \partial B(0,R) $, y siempre existe $t> t_0$ tal que  $t\in A$}.%$|\varphi(t)|\in \overline{B(0,R)}$.% 
 		\begin{itemize}
   		\item Supongamos que $\varphi_\alpha(t_0)\in B(0,R)$.
     \end{itemize}
En el problema \eqref{eq:Problema 0} podemos tomar como condición inicial $\varphi(0)=\varphi_\alpha(t_0)\replaced{=:}{=}\beta\in B(0,R)$, entonces por el Teorema \ref{P-L} existe una solución, que \comment{en español ``notaremos'' solo tiene el significado de ``percibir algo''} notaremos  $\varphi_{\beta}$. Luego, tenemos que para todo $t\in [0,\delta)$ \normalmarginpar\comment{Nunca introdujiste $\delta$} el par $(t,\varphi_\beta(t))\in \Omega$, donde  $\Omega$ es un entorno \added{????} de $[0,T]\times B(0,R)$. Ahora podemos extender a $\varphi_\alpha $ de la siguiente manera
...
Por lo tanto para todo $t\in[t_0,t_0+\delta)$ vale que $|\varphi_\alpha(t)|=|\varphi_\beta(t-t_0)|\leq R$. \replaced{Pero esto contradice que $t_0$ es límite de una sucesión en $[0,T]\setminus A$.}{Es decir existe $t>t_0$ tal que $t\in A$. }
...
Por \replaced{Definición \ref{fun-tele}}{\eqref{fun-tele}}  $x_1=\varphi(t_0)+f(t,\varphi(t_0))\mu(\{t_0\})\in B(0,R)$\replaced {.  Consideremos }{, y tomando} la medida $\hat{\mu}=\mu-\mu(\{t_0\})\delta_{t_0}$\replaced{. Es}{,  es} claro que $\hat{\mu}(\{t_0\})=0$, \replaced{ por consiguiente podemos}{ puedo} aplicar el Teorema \ref{P-L} para la condición inicial $\varphi(0)=x_1$, pero con la medida $\hat{\mu}$.  Luego, existe $\delta>0$  y $\varphi_{x_1}$ \added{Solución???}  tal que $|\varphi_{x_1}(t)|\leq R$ para todo $t\in [0,0+\delta)$. Ahora puedo extender la solución $\varphi_\alpha$ al intervalo $[0,t_0+\delta)$  de la siguiente manera
...
Por lo tanto existe $t>t_0$ tal que $|\varphi_\alpha(t)|<R$, es decir vale que $t\in A$. \comment{Me gustaría completar un poco más: Por qué es solución? Por qué  existe $t>t_0$ tal que $|\varphi_\alpha(t)|<R$?} 
...
\begin{itemize}
\item Supongamos $\varphi(t_0)\in \partial B(0,R)$ y $\mu(\{t_0\})= 0$ \reversemarginpar\comment{Pregunta para que nos hagamos, la continuidad de $f$ respecto a $t$, es sólo requerida cuando $\mu(\{t\})=0$. Me parece que puede ser importante}
\end{itemize}
...
\end{equation}
El término de la derecha es una expresión cuadrática respecto a $\mu((t_0,t))$, y tiene como raíces $t_0$ \comment{Las raíces de la función cuadrática tendrías que pensarlas en el lugar de $\mu((t_0,t))$ no en el de $t$}  y  otro valor a derecha de $t_0$, es decir existe $t_1>t_0$ tal que $|\varphi_\alpha(t_1)|^2-R^2\leq 0$, por lo tanto $t_1\in A$.
...
Luego, para cualquier $\alpha\in\overline{B(0,R)}$, $$P(\alpha)=\varphi_\alpha(T)\in\overline{B(0,R)}.$$ 
 ...
 \highlight{Algo presentando el siguiente resultado. La referencia también}
 ...
 \begin{thm} \label{th:final}
 	Sea $\mu$ una medida de Borel finita, y sea $f:[0,T]\times\rr^n\to\rr^n$ una función que cumple con las condiciones \ref{pm1} a \ref{pm4}. Además, \added{supongamos que} existe una bola $B(0,R)$ tal que si $x\in\overline{B(0,R)}$, entonces
    $x+f(t,x)\mu(\{t\})\in \overline{B(0,R)}$ para todo $t\in[0,T]$, y
 	...
 	\end{equation} 
tiene al menos una solución. \added{Tengo la impresión que el problema lo escribiste varias veces en la tesis} 
\end{thm}
 \begin{proof}[Dem.]
...
A este sistema de ecuaciones lo podemos escribir de manera vectorial, tomando $\varphi=(u,v)$, $G_1(t,\varphi)=(v,g(t,u,v))$ y $G_2(t,\varphi)=(0,g_2(t,u,v))$.
 ...
 \varphi(0)=\varphi(T).
	\end{array}\right. 
\end{equation*} 
Si llamamos $F(t,\varphi)=\left( G_1(t,\varphi)\chi_{[0,T]-\{t_1,..,t_r\}}+G_2(t,\varphi)\chi_{\{t_1,...,t_r\}}\right) $ y tomando $d\mu=d\left(\lambda+\displaystyle\sum_{i=1}^r\delta_{t_i}\right)$, entonces la ecuación vectorial se transforma en la del Teorema \ref{th:final}. \comment{No habíamos desarrollado un ejemplo más concreto?}
