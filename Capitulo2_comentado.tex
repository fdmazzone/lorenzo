...
\begin{defi}
	Dada $u:[0,T]\to \rr$, definimos la variación \index{variación!} de $u$ como \index[Simbolo]{$\V(u,[0,T])$}\label{def:variación}
	$$\V(u,[0,T])=\sup_{\substack{ t_i\in [0,T]\\
			0\leq \textrm{\todo{Pondría $0=t_1$ y $t_n=T$}}t_1 < t_2< \dots < t_n  \leq T}}\left\lbrace \sum_{i=1}^{n-1}|u(t_{i+1})-u(t_i)|\right\rbrace $$
...
Por lo tanto $d(u,v)\leq d(u,w)+d(w,v)$.
    \end{itemize}
Veamos que $\left(BV_\alpha([0,T],\rr^n),d\right)$ es completo. Sea $\{u_n\}$ una sucesión de Cauchy,  luego existe una constante $K$ tal que $\forall n$ $d(u_n,0)=V(u_n,[0,T])\leq K$. Además por \ref{lem:var-acotada} para cualquier $t\in[0,T]$
\begin{equation} \label{eq:desigualdad d}
|u_n(t)|\leq|u_n(t)-u_n(0)|+|u_n(0)|\leq V(u_n,[0,T])+|\alpha|\leq K.
\end{equation}
Luego por el teorema de Helly's \cite{Natanson} existe una subsecesión convergente, y por lo tanto la sucesión es convergente.}
\end{proof}
...
Vamos a definir la medida de \replaced{Lebesgue}{Labesgue}-Stieltjes para una función de creciente y continua a izquierda. El mismo desarrollo se puede hacer para funciones decrecientes, con la salvedad de que la medida generada en negativa.


\begin{defi}\label{def:premedida}
Sea $\mathcal{A}$ un álgebra. Diremos que la función $\mu_0:\mathcal{A}\to [0,\infty)$ es una premedida\index{premedida}, si cumple 
\begin{itemize}
